...
import {getBlockInput, ImportBlockOpts, BlockInput} from "../../../../chain/blocks/types.js";
  ...
  return {
    ...
    async publishBlock(signedBlockOrContents, opts?: ImportBlockOpts) {
      ...
      if ((signedBlockOrContents as allForks.SignedBlockContents).signedBlock !== undefined) {
        ...
        signedBlobs = [];
        blockForImport = getBlockInput.preDeneb(config, signedBlock);
      }
      ...
      const publishPromises = [
        ...
        ...signedBlobs.map((signedBlob) => () => network.gossip.publishBlobSidecar(signedBlob)),
      ];
